          ...
          <div class="card-body">
            <form action="{{url('/list-pelatihan')}}" method="post" enctype="multipart/form-data">
                ...
                <div class="form-group my-3">
                    ...
                    <input value="{{$data->judul}}" type="text" name="judul" placeholder="Input judul..." class="form-control">
                </div>
                <div class="form-group my-3">
                    <p class="my-0 fw-bold">Instruktur Pelatihan<span class="text-danger">*</span></p>
                    <input value="{{$data->instruktur}}" type="text" name="instruktur" placeholder="Input instruktur..." class="form-control">
                </div>
                <div class="form-group my-3">
                    <p class="my-0 fw-bold">Tanggal Pelatihan<span class="text-danger">*</span></p>
                    <input value="{{$data->tanggal}}" type="date" name="tanggal" placeholder="Input tanggal..." class="form-control">
                </div>
                <div class="form-group my-3">
                    <p class="my-0 fw-bold">Deskripsi Pelatihan<span class="text-danger">*</span></p>
                    <textarea name="deskripsi" class="form-control" col="5">{{$data->deskripsi}}</textarea>
                </div>
                <div class="form-group my-3">
                    <p class="my-0 fw-bold">Biaya Pelatihan<span class="text-danger">*</span></p>
                    <input value="{{$data->biaya}}" type="number" name="biaya" placeholder="Input biaya..." class="form-control">
                </div>
                <div class="form-group my-3">
                ...
                <div class="my-3 form-group">
                    ...
                    <select name="kategori" class="form-select" id="kategori">
                        ...
                        <option value="sop" @if($data->kategori == 'sop') selected @endif>SOP</option>
                        <option value="spm" @if($data->kategori == 'spm') selected @endif>SPM</option>
                        <option value="course" @if($data->kategori == 'course') selected @endif>Course</option>
                        <option value="materi_umum" @if($data->kategori == 'materi_umum') selected @endif>Materi Umum</option>
                    </select>
                </div>
                <div class="mb-3 form-group">
                    ...
                    <select name="sertifikat" class="form-select" id="sertifikasi">
                        <option selected disabled>-- Pilih --</option>
                        <option value="skp"  @if($data->sertifikat == 'skp') selected @endif>Bersertifikat (SKP)</option>
                        <option value="non_skp"  @if($data->sertifikat == 'non_skp') selected @endif>Tidak Bersertifikat</option>
                    </select>
                </div>
                @if($data->sertifikat == 'non_skp')
                ...
                @else
                <div class="form-group my-3" id="link">
                    <p class="my-0 fw-bold">Link Pelatihan Kemenkes<span class="text-danger">*</span></p>
                    <input type="url" value="{{$data->link_lms_kemenkes}}" name="link_lms_kemenkes" placeholder="Input Link..." class="form-control">
                </div>
                @endif
